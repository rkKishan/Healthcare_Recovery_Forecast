...
# runtime is ever escaped, and nothing here needs those privileges. uid 1000 is
# the conventional first non-system user, which some hosts assume.
RUN useradd --create-home --uid 1000 appuser \
    && chown -R appuser:appuser /app
USER appuser

# Render, Fly and Cloud Run all inject the port to listen on as $PORT and
# route to it; EXPOSE is only metadata. The CMD honours $PORT wherever it is
# set and falls back to 2800, so a bare `docker run -p 2800:2800` still works.
EXPOSE 2800

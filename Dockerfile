...
HEALTHCHECK --interval=30s --timeout=5s --start-period=40s --retries=3 \
    CMD python -c "import urllib.request,sys; sys.exit(0 if urllib.request.urlopen('http://localhost:2800/api/health', timeout=4).status == 200 else 1)"
...
CMD ["sh", "-c", "flask --app 'backend.app:create_app' db upgrade && exec gunicorn --bind 0.0.0.0:2800 --workers 2 --timeout 120 'backend.app:create_app()'"]
